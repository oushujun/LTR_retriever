...
while (<List>){
	next if /^#/;
	next if /^\s+/;
	my ($chr, $element_start, $element_end, $element_length, $lLTR_start, $lLTR_end, $lLTR_length, $rLTR_start, $rLTR_end, $rLTR_length, $seq_ID, $loc, $motif, $TSD, $lTSD, $rTSD, $IN, $sim);
	($loc, undef, $motif, $TSD, $lTSD, $rTSD, $IN, $sim) = (split);
...
my $chr_ori=$chr;
	...
	print GFF "$chr\t$annotator\tlong_terminal_repeat\t$lLTR_start\t$lLTR_end\t.\t?\t.\tParent=LTR_retrotransposon$i\n";
	print GFF "$chr\t$annotator\tlong_terminal_repeat\t$rLTR_start\t$rLTR_end\t.\t?\t.\tParent=LTR_retrotransposon$i\n";
	print GFF "$chr\t$annotator\ttarget_site_duplication\t$rTSD\t.\t?\t.\tParent=repeat_region$i\n" unless $TSD eq "NA";
	print GFF "###\n";
	$i++;
	}
